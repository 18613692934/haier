...
//        print_r("begintime:".date("Y-m-d H:i:s",$all)."\n\n");
//        print_r("endTime:".date("Y-m-d H:i:s",$endTime)."\n\n");
//        print_r("min_time:".date("Y-m-d H:i:s",$min_time)."\n\n");
//        print_r("*******************************************");
	$select->execute();
			...
			if($item){/*汇总表中有这条信息，就进行更新操作*/
				...
				foreach ($result as $key => $value) {
					...
					$value['endtime'] = $endTime;
                                        $value['startdate'] = date("Y-m-d H:i:s",$all);
					$value['enddate'] = date("Y-m-d H:i:s",$endTime);
					if( 0 == $value['avg_temp']){
						$value['avg_temp'] = "";
					}
					if( 0 == $value['avg_hum']){
...
function getTime($now,$q){
    ...
    switch ($q) {
        ...
        case "week":
            ...
            $monday_str = $now - ($now_day-1)*60*60*24;
            $monday = strtotime(date('Y-m-d 00:00:00', $monday_str));
            //获取一周的最后一天，注意最后一天是星期六
            $sunday_str = $now + (7-$now_day)*60*60*24;
            $sunday = strtotime(date('Y-m-d 23:59:59', $sunday_str));
            $beginTime = $monday; 
